...
class BarcodeAnalyser(
    private val onBarcodeScanned: (String) ->Unit ): ImageAnalysis.Analyzer {
    private val supportedImageFormats = listOf(
        ImageFormat.YUV_420_888,
        ImageFormat.YUV_422_888,
        ImageFormat.YUV_444_888
        ...
        if (image.format in supportedImageFormats) {
            ...
            try {
                val results = MultiFormatReader().apply {
                    //map of all supported types we want to scan
                    setHints(
                        mapOf(
                            /*TODO Barcode schwieriger als QRCode, muss horizontal sein, evtl in landscape mode
                              muss erst image drehen, wahrechilich über image.imageInfo.rotationDegree -> berechnung um horizontal zu drehen */
                ...
                onBarcodeScanned(results.text/*TODO foodfacts API ansprechen, entsprechendes lebensmittel suchen und ausgeben*/)

            } catch (e: Exception) { //exception jedes mal code nicht im frame -> abfangen da häufig
                e.printStackTrace()
            } finally {
                image.close()
            }
